...
if (valorBruto >= 0 && valorBruto <= 1100) {
    const salarioRecebido = (valorBruto - (valorBruto * 0.05)) + valorBeneficio;
    print(salarioRecebido.toLocaleString('pt-br', {style: 'currency', currency:'BRL'}))
} else if (valorBruto >= 1100.01 && valorBruto <= 2500) {
    const salarioRecebido = (valorBruto - (valorBruto * 0.1)) + valorBeneficio;
    print(salarioRecebido.toLocaleString('pt-br', {style: 'currency', currency:'BRL'}))
} else if (valorBruto > 2500) {
    const salarioRecebido = (valorBruto - (valorBruto * 0.15)) + valorBeneficio;
    print(salarioRecebido.toLocaleString('pt-br', {style: 'currency', currency:'BRL'}))
}
